...
assert not hasattr(a, "__next__") #It's not an itterator. 
assert     hasattr(a, "__iter__") #You can turn it into an itterator.    
s = 0                             #All iterators are iterable, but not all iterable objects are iterators. 
for v in a :
    s += v
assert s == 9
...
a = [2, 3, 4]
for v in a :
    v += 1            # This is the local object, not the object stored in the list, ints are immutable in a list. 
assert a == [2, 3, 4]

a = [[2], [3], [4]]
for v in a :
    v += (5,)                        # Lists in a list are mutable
assert a == [[2, 5], [3, 5], [4, 5]]

a = [(2,), (3,), (4,)]
for v in a :
    v += (5,)                  # tuples are immutable
assert a == [(2,), (3,), (4,)]

a = ["abc", "def", "ghi"]
for v in a :
    v += "x"                      # Strings in a list are immutable
assert a == ["abc", "def", "ghi"]

a = ["abc", "def", "ghi"]
s = ""
for v in a :
    s += v    # you can add strings, it concantonates. 
assert s == "abcdefghi"
...
assert set(k) == {2, 3, 4}
assert set(k) == {2, 3, 4} #sets are iterable but not iterators
...
x = range(10)
assert hasattr(x, "__getitem__")   #range is indexable
assert x[0] == 0
assert x[9] == 9
try :
