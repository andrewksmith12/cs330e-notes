...
s = "b ab\naab 123"
a = re.split("^a", s)      # start of string  SPLIT DOESN'T DO MULTILINE INTERPRETATION
assert isinstance(a, list)
assert a == [s]

s = "b ab\naab 123"
r = re.compile("^a", re.M)                          # multiline  COMPILE SPLIT *DOES* DO MULTILINE. Compile recognizes /n as an event not a character? lolidk
...
s = "b ab\naab 123"
a = re.split(r"\d", s)                 # any digit  the r allows you to recognize the special regex characters. "Raw string notation."
assert isinstance(a, list)
assert a == ['b ab\naab ', '', '', '']
...
assert m.group(2) == " "
assert m.group(3) == "a"
    #[^a] <- matches on anything that's not a.  
s = "b ab\naab 123"
...
s = "b ab\naab 123"
t = re.sub("b.", "xx", s) # . means any character
assert s == "b ab\naab 123"
assert t == "xxab\naaxx123"
